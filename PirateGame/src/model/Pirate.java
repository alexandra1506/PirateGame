...
public class Pirate extends Creature implements UsableOnMap, EventListener {
    ...
    private EventEmit eventEmit; // интерфейс, который вызывает событие( например, изменение координат)

    public Pirate(Coordinate coordinate, int stamina, String name, EventEmit eventEmit) {
        super(coordinate, stamina);
        this.name = name;
        this.eventEmit = eventEmit;
        this.eventEmit.AddNewListener("ADD_NEW_OBJECT", this);
    }
        ...
        switch (eventName){
            case "ADD_NEW_OBJECT":
                ...
                if (this == parameter){
                    ...
                    destination = new Coordinate(((UsableOnMap)parameter).X(), ((UsableOnMap)parameter).Y());
                }
                break;
            default:
    ...
    public void doStep() throws OutOfMapException {
      ...
      if (destination != null){
          ...
          change.setWere(new Coordinate(self.getX(), self.getY())); // были
          change.setObj(this);
          int step;
